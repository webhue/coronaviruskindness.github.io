...
d3.csv(publishedData, function (d) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (data[i].contribute != '') {
      ...
      contributeHtml =
        "<br><br>Want to help? &nbsp; <a href='" +
        data[i].contribute +
        "' target='_blank'><button class='button-small'><b>Contribute</b></button></a>";

      // Append this article to the contribution DIV
      document.getElementById('contribute').innerHTML +=
        "<tr><td><a href='" +
        data[i].contribute +
        "' target='_blank'><button class='button'><b>Contribute</b></button></a></td><td id='towrap' word-wrap='break-word'><b><a href='" +
        data[i].url +
        "' target='_blank'>" +
        data[i].title +
        '</a></b></td></tr><br>';
    ...
    // Create the Marker!
    L.marker([data[i].lat, data[i].lon], { icon: heartMarker })
      .addTo(mymap)
      .bindPopup(
        "<b><a href='" +
          data[i].url +
    ...
    var totalaok = i + 1;
    document.getElementById('total-aok').innerHTML =
      '<b>Number of Acts of Kindness: <font color="#c1006e" size="4">' +
      totalaok.toString() +
      '</font></b>';
  }
